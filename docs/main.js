...
{
	if (region.H.av === region.D.av)
	{
		return 'on line ' + region.H.av;
	}
	return 'on lines ' + region.H.av + ' through ' + region.D.av;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { c4: 'hidden', cQ: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { c4: 'mozHidden', cQ: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { c4: 'msHidden', cQ: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { c4: 'webkitHidden', cQ: 'webkitvisibilitychange' }
		: { c4: 'hidden', cQ: 'visibilitychange' };
}
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {bz: fragment, bE: host, bW: path, bY: port_, b0: protocol, b1: query};
	});
...
var $author$project$Styles$Dark = 0;
var $author$project$Main$Index = 0;
var $author$project$Main$Model = F6(
	function (key, dimensions, url, style, page, styleAnimations) {
		return {au: dimensions, bL: key, Z: page, a: style, ad: styleAnimations, A: url};
	});
var $author$project$Main$NotFound = 4;
var $author$project$Main$Projects = 3;
var $author$project$Main$Resume = 1;
var $author$project$Main$ResumeExport = 2;
var $elm$browser$Browser$Navigation$load = _Browser_load;
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $elm$url$Url$Parser$State = F5(
	function (visited, unvisited, params, frag, value) {
		return {W: frag, _: params, Q: unvisited, I: value, af: visited};
	});
		...
		return $elm$url$Url$Parser$getFirstMatch(
			parser(
				A5(
					...
					_List_Nil,
					$elm$url$Url$Parser$preparePath(url.bW),
					$elm$url$Url$Parser$prepareQuery(url.b1),
					url.bz,
					$elm$core$Basics$identity)));
	});
var $author$project$Main$IndexRoute = {$: 0};
var $author$project$Main$ProjectsRoute = {$: 3};
var $author$project$Main$ProjectsViewRoute = function (a) {
	return {$: 4, a: a};
};
var $author$project$Main$ResumeExportRoute = {$: 2};
var $author$project$Main$ResumeRoute = {$: 1};
...
var $author$project$Main$parser = $elm$url$Url$Parser$oneOf(
	_List_fromArray(
		[
			A2($elm$url$Url$Parser$map, $author$project$Main$IndexRoute, $elm$url$Url$Parser$top),
			A2(
			$elm$url$Url$Parser$map,
			$author$project$Main$ResumeRoute,
			$elm$url$Url$Parser$s('resume')),
			A2(
			$elm$url$Url$Parser$map,
			$author$project$Main$ResumeExportRoute,
			A2(
				$elm$url$Url$Parser$slash,
				$elm$url$Url$Parser$s('resume'),
				$elm$url$Url$Parser$s('export'))),
			A2(
			$elm$url$Url$Parser$map,
			...
			A2(
			$elm$url$Url$Parser$map,
			$author$project$Main$ProjectsViewRoute,
		...
		if (!_v0.$) {
			switch (_v0.a.$) {
				...
				default:
					...
					return _Utils_Tuple2(
						model,
						$elm$browser$Browser$Navigation$load('/projects/' + (project + '/index.html')));
			}
		} else {
			return _Utils_Tuple2(
				_Utils_update(
...
var $author$project$Main$init = F3(
	function (keys, url, key) {
		return A2(
			$author$project$Main$gotoUrl,
			url,
			A6($author$project$Main$Model, key, keys, url, 0, 0, 0));
	});
...
var $author$project$Main$SwitchThemeDone = function (a) {
	return {$: 5, a: a};
};
var $author$project$Styles$Light = 1;
var $author$project$Styles$flip = function (style) {
	if (!style) {
		return 1;
	} else {
		return 0;
...
var $author$project$Main$openInternalUrl = F2(
	function (key, url) {
		var _v0 = A2($elm$url$Url$Parser$parse, $author$project$Main$parser, url);
		if ((!_v0.$) && (_v0.a.$ === 4)) {
			var project = _v0.a.a;
			return $elm$browser$Browser$Navigation$load('/projects/' + (project + '/index.html'));
		} else {
			return A2(
...
var $author$project$Ports$sendMessage = _Platform_outgoingPort('sendMessage', $elm$json$Json$Encode$string);
var $author$project$Ports$printPage = $author$project$Ports$sendMessage('print');
var $elm$core$Process$sleep = _Process_sleep;
var $author$project$Main$styleSwitchDuration = 500;
		...
		switch (msg.$) {
			case 0:
				...
				if (!urlRequest.$) {
					...
					return _Utils_Tuple2(
						model,
						A2($author$project$Main$openInternalUrl, model.bL, url));
				} else {
			...
			case 4:
				return _Utils_Tuple2(
					...
					A2(
						...
						$author$project$Main$SwitchThemeDone,
						A2(
							$elm$core$Task$andThen,
							function (_v2) {
								return $elm$core$Task$succeed(model.ad + 1);
							},
							$elm$core$Process$sleep($author$project$Main$styleSwitchDuration))));
			default:
				var count = msg.a;
				if (_Utils_eq(count, model.ad)) {
					var style = (!(count % 2)) ? model.a : $author$project$Styles$flip(model.a);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{a: style, ad: 0}),
...
var $author$project$Styles$barColor = function (style) {
	if (!style) {
		...
		return A3($mdgriffith$elm_ui$Element$rgb255, 231, 83, 24);
	}
};
...
var $author$project$Styles$bar = F2(
		...
		return _Utils_ap(
			_List_fromArray(
				[
					...
					$author$project$Styles$barColor(style))
				]),
			attributes);
	});
...
var $mdgriffith$elm_ui$Internal$Model$asEl = 2;
var $mdgriffith$elm_ui$Internal$Style$classes = {cp: 'a', aZ: 'atv', cr: 'ab', cs: 'cx', ct: 'cy', cu: 'acb', cv: 'accx', cw: 'accy', cx: 'acr', bm: 'al', bn: 'ar', cy: 'at', a_: 'ah', a$: 'av', cA: 's', cE: 'bh', cG: 'b', cI: 'w7', cK: 'bd', cL: 'bdt', aD: 'bn', cM: 'bs', aE: 'cpe', cR: 'cp', cS: 'cpx', cT: 'cpy', K: 'c', aG: 'ctr', aH: 'cb', aI: 'ccx', L: 'ccy', at: 'cl', aJ: 'cr', cU: 'ct', cW: 'cptr', cX: 'ctxt', c1: 'fcs', by: 'focus-within', c2: 'fs', c3: 'g', a4: 'hbh', a5: 'hc', bB: 'he', a6: 'hf', bC: 'hfp', c5: 'hv', c8: 'ic', da: 'fr', aN: 'lbl', dc: 'iml', dd: 'imlf', de: 'imlp', df: 'implw', dg: 'it', di: 'i', bN: 'lnk', am: 'nb', bQ: 'notxt', dm: 'ol', dn: 'or', Y: 'oq', ds: 'oh', Z: 'pg', bV: 'p', dt: 'ppe', dv: 'ui', b6: 'r', dx: 'sb', dy: 'sbx', dz: 'sby', dA: 'sbt', dC: 'e', dD: 'cap', dE: 'sev', dI: 'sk', cd: 't', dL: 'tc', dM: 'w8', dN: 'w2', dO: 'w9', dP: 'tj', aU: 'tja', dQ: 'tl', dR: 'w3', dS: 'w5', dT: 'w4', dU: 'tr', dV: 'w6', dW: 'w1', dX: 'tun', cg: 'ts', ae: 'clr', d0: 'u', bi: 'wc', cm: 'we', bj: 'wf', cn: 'wfp', bk: 'wrp'};
...
var $mdgriffith$elm_ui$Internal$Model$columnClass = $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.K);
var $mdgriffith$elm_ui$Internal$Model$gridClass = $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.c3);
var $mdgriffith$elm_ui$Internal$Model$pageClass = $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.Z);
...
var $mdgriffith$elm_ui$Internal$Model$rowClass = $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.b6);
var $mdgriffith$elm_ui$Internal$Model$singleClass = $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dC);
...
var $mdgriffith$elm_ui$Internal$Model$getStyleName = function (style) {
	switch (style.$) {
		...
		case 9:
			var pos = style.a;
			return 'gp grid-pos-' + ($elm$core$String$fromInt(pos.b6) + ('-' + ($elm$core$String$fromInt(pos.bs) + ('-' + ($elm$core$String$fromInt(pos.aC) + ('-' + $elm$core$String$fromInt(pos.aM)))))));
		case 11:
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.c5, $mdgriffith$elm_ui$Internal$Style$classes.ae)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.aZ, $mdgriffith$elm_ui$Internal$Style$classes.ae)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
...
var $mdgriffith$elm_ui$Internal$Style$overrides = '@media screen and (-ms-high-contrast: active), (-ms-high-contrast: none) {' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.cA) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.b6) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.cA) + (' { flex-basis: auto !important; } ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.cA) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.b6) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.cA) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.aG) + (' { flex-basis: auto !important; }}' + ($mdgriffith$elm_ui$Internal$Style$inputTextReset + ($mdgriffith$elm_ui$Internal$Style$sliderReset + ($mdgriffith$elm_ui$Internal$Style$trackReset + ($mdgriffith$elm_ui$Internal$Style$thumbReset + $mdgriffith$elm_ui$Internal$Style$explainer)))))))))))))));
...
var $mdgriffith$elm_ui$Internal$Style$emptyIntermediate = F2(
	function (selector, closing) {
		return {aF: closing, m: _List_Nil, O: _List_Nil, y: selector};
	});
					...
					case 3:
						...
						return _Utils_update(
							rendered,
							{
								m: A2(
									$elm$core$List$cons,
									{aF: '\n}', m: _List_Nil, O: props, y: '@supports (' + (prop + (':' + (value + (') {' + parent.y))))},
									rendered.m)
							});
					case 5:
						var selector = rule.a;
...
var $mdgriffith$elm_ui$Internal$Style$renderCompact = function (styleClasses) {
	...
	var renderClass = function (rule) {
		...
		if (!_v2.b) {
			...
			return rule.y + ('{' + (renderValues(rule.O) + (rule.aF + '}')));
		}
	};
		...
		switch (rule.$) {
			...
			case 9:
				...
				var modernPosition = A2(
					...
					_List_fromArray(
						[
							'grid-row: ' + ($elm$core$String$fromInt(position.b6) + (' / ' + ($elm$core$String$fromInt(position.b6 + position.aM) + ';'))),
							'grid-column: ' + ($elm$core$String$fromInt(position.bs) + (' / ' + ($elm$core$String$fromInt(position.bs + position.aC) + ';')))
						]));
				var _class = '.grid-pos-' + ($elm$core$String$fromInt(position.b6) + ('-' + ($elm$core$String$fromInt(position.bs) + ('-' + ($elm$core$String$fromInt(position.aC) + ('-' + $elm$core$String$fromInt(position.aM)))))));
	...
	function (name, modifier, _v0) {
		...
		return _List_fromArray(
			[
				...
				A2($mdgriffith$elm_ui$Internal$Model$bracket, '.' + (name + ('.' + (modifier + ('> .' + ($mdgriffith$elm_ui$Internal$Style$classes.cd + (', .' + (name + (' .' + (modifier + (' > .' + $mdgriffith$elm_ui$Internal$Style$classes.cd)))))))))), textAdjustment)
			]);
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2(
					$mdgriffith$elm_ui$Internal$Model$bracket,
					'.' + (name + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.dD + ('> .' + ($mdgriffith$elm_ui$Internal$Style$classes.cd + (', .' + (name + (' .' + ($mdgriffith$elm_ui$Internal$Style$classes.dD + (' > .' + $mdgriffith$elm_ui$Internal$Style$classes.cd)))))))))),
					_List_fromArray(
						[
...
var $mdgriffith$elm_ui$Internal$Model$adjust = F3(
	function (size, height, vertical) {
		return {aM: height / size, an: size, ci: vertical};
	});
...
var $mdgriffith$elm_ui$Internal$Model$convertAdjustment = function (adjustment) {
	var lines = _List_fromArray(
		[adjustment.cP, adjustment.cD, adjustment.cY, adjustment.dj]);
	var lineHeight = 1.5;
					...
					return A3(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class($mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dC))
									]))
							]));
			}
		}();
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.cA, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.aG, $mdgriffith$elm_ui$Internal$Style$classes.L, $mdgriffith$elm_ui$Internal$Style$classes.cx])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.cA, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.aG, $mdgriffith$elm_ui$Internal$Style$classes.L, $mdgriffith$elm_ui$Internal$Style$classes.cv])))
						]),
					_List_fromArray(
						[html])) : html));
			case 1:
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.cA, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.aG, $mdgriffith$elm_ui$Internal$Style$classes.cw])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.cA, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.aG, $mdgriffith$elm_ui$Internal$Style$classes.cu])))
						]),
					_List_fromArray(
						[html])) : html));
			default:
...
var $mdgriffith$elm_ui$Internal$Model$textElementClasses = $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.cd + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.bi + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.a5)))));
...
var $mdgriffith$elm_ui$Internal$Model$textElementFillClasses = $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.cd + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.bj + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.a6)))));
						...
						switch (location) {
							case 0:
								return A2(
									...
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.am, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.cp]));
							case 1:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.am, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.cG]));
							case 2:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.am, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.dn]));
							case 3:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.am, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.dm]));
							case 4:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.am, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.da]));
							default:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.am, $mdgriffith$elm_ui$Internal$Style$classes.dC, $mdgriffith$elm_ui$Internal$Style$classes.cE]));
						}
					}())
				]),
			_List_fromArray(
...
var $mdgriffith$elm_ui$Internal$Model$alignXName = function (align) {
	switch (align) {
		case 0:
			return $mdgriffith$elm_ui$Internal$Style$classes.a_ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.bm);
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.a_ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.bn);
		default:
			...
			return (portion === 1) ? _Utils_Tuple3(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Internal$Style$classes.cA + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.K + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
			...
			return (portion === 1) ? _Utils_Tuple3(
				...
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
						$mdgriffith$elm_ui$Internal$Style$classes.cA + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.b6 + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
		...
		return A4(
			...
			A2(
				...
				A2(
					...
					A2(
						$elm$core$List$cons,
						$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.aI + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.L + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.dA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.bQ)))))),
						A2(
							$elm$core$List$cons,
...
var $author$project$Styles$buttonColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 76, 175, 80);
	} else {
		return A3($mdgriffith$elm_ui$Element$rgb255, 241, 93, 34);
	}
};
var $author$project$Styles$buttonHoverColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 92, 207, 96);
	} else {
		return A3($mdgriffith$elm_ui$Element$rgb255, 255, 123, 54);
	}
};
var $author$project$Styles$buttonPressColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 60, 159, 64);
	} else {
		return A3($mdgriffith$elm_ui$Element$rgb255, 211, 63, 14);
	}
};
...
var $author$project$Styles$fgColor = function (style) {
	if (!style) {
		...
		return _Utils_ap(
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$Background$color(
					$author$project$Styles$buttonColor(style)),
					$mdgriffith$elm_ui$Element$Font$color(
					$author$project$Styles$fgColor(style)),
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Background$color(
							$author$project$Styles$buttonPressColor(style))
						])),
					$mdgriffith$elm_ui$Element$mouseOver(
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Background$color(
							$author$project$Styles$buttonHoverColor(style))
						])),
				...
				A2(
					...
					A2(
						...
						A2(
							...
							A2(
								$elm$core$List$cons,
								$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.aI + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.L + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.bN)))),
								attrs))))),
			$mdgriffith$elm_ui$Internal$Model$Unkeyed(
				_List_fromArray(
					[label])));
...
var $mdgriffith$elm_ui$Element$row = F2(
	function (attrs, children) {
		return A4(
			...
			A2(
				$elm$core$List$cons,
				$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.at + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.L)),
				A2(
					$elm$core$List$cons,
					$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$shrink),
					A2(
...
var $author$project$ElementFix$text = function (str) {
	return $mdgriffith$elm_ui$Element$html(
		$elm$html$Html$text(str));
};
var $author$project$Main$bar = F2(
	function (model, contextActions) {
		var _v0 = function () {
			var _v1 = model.a;
			if (!_v1) {
				return _Utils_Tuple2(
					...
					_List_fromArray(
						[
							...
							A2($elm$html$Html$Attributes$style, 'filter', 'invert(90%)'))
						]));
			} else {
				return _Utils_Tuple2('/assets/moon.png', _List_Nil);
			}
		}();
		var styleIcon = _v0.a;
		var styleAttributes = _v0.b;
		return A2(
			$mdgriffith$elm_ui$Element$row,
			A2($author$project$Styles$bar, model.a, _List_Nil),
			_List_fromArray(
				[
					A2(
					$mdgriffith$elm_ui$Element$link,
					A2($author$project$Styles$button, model.a, _List_Nil),
					{
						u: $author$project$ElementFix$text('Home'),
						A: '/'
					}),
					A2(
					$mdgriffith$elm_ui$Element$link,
					A2($author$project$Styles$button, model.a, _List_Nil),
					{
						u: $author$project$ElementFix$text('Projects'),
						A: '/projects'
					}),
					A2(
					$mdgriffith$elm_ui$Element$link,
					A2($author$project$Styles$button, model.a, _List_Nil),
					{
						u: $author$project$ElementFix$text('Resume'),
						A: '/resume'
					}),
					A2(
					$mdgriffith$elm_ui$Element$row,
					...
					A2($author$project$Styles$button, model.a, _List_Nil),
					{
						u: A2(
							...
							_Utils_ap(
								...
								styleAttributes),
							{V: 'Dark/Light mode switch', P: styleIcon}),
						bT: $elm$core$Maybe$Just($author$project$Main$SwitchTheme)
					})
				]));
	});
...
var $mdgriffith$elm_ui$Element$column = F2(
	function (attrs, children) {
		return A4(
			...
			A2(
				$elm$core$List$cons,
				$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.cU + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.at)),
				A2(
					$elm$core$List$cons,
					$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$shrink),
					A2(
...
var $author$project$Resume$bgColor = A3($mdgriffith$elm_ui$Element$rgb255, 255, 255, 255);
var $author$project$Experience$about = _List_fromArray(
	[
		_List_fromArray(
...
var $mdgriffith$elm_ui$Element$Font$bold = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$fontWeight, $mdgriffith$elm_ui$Internal$Style$classes.cI);
var $author$project$Resume$fgColor = A3($mdgriffith$elm_ui$Element$rgb255, 40, 40, 40);
...
var $author$project$Resume$bodyPart = F2(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$author$project$ElementFix$text(title)
						])),
					content
				]));
	});
...
var $author$project$Experience$educationList = _List_fromArray(
	[
		...
		D: $elm$core$Maybe$Just(
			A3($justinmimbs$date$Date$fromCalendarDate, 2017, 9, 1)),
		bI: 'Nizhniy Novgorod Technical College',
		E: 'Nizhniy Novgorod, Russia',
		H: A3($justinmimbs$date$Date$fromCalendarDate, 2013, 9, 1)
	}
	]);
...
var $justinmimbs$date$Date$toCalendarDate = function (_v0) {
	...
	return A3($justinmimbs$date$Date$toCalendarDateHelp, date.co, 0, date.bb);
};
...
var $justinmimbs$date$Date$toWeekDate = function (_v0) {
	...
	return {
		ck: 1 + (((rd - week1Day1) / 7) | 0),
		cl: wy,
		d3: $justinmimbs$date$Date$numberToWeekday(wdn)
	};
};
...
var $elm$parser$Parser$Advanced$DeadEnd = F4(
	function (row, col, problem, contextStack) {
		return {bs: col, cV: contextStack, bZ: problem, b6: row};
	});
...
var $elm$parser$Parser$Advanced$fromState = F2(
	function (s, x) {
		return A2(
			...
			A4($elm$parser$Parser$Advanced$DeadEnd, s.b6, s.bs, x, s.d));
	});
...
var $elm$parser$Parser$Advanced$token = function (_v0) {
	...
	return function (s) {
		var _v1 = A5($elm$parser$Parser$Advanced$isSubString, str, s.b, s.b6, s.bs, s.P);
		var newOffset = _v1.a;
		var newRow = _v1.b;
		var newCol = _v1.c;
		return _Utils_eq(newOffset, -1) ? A2(
			...
			progress,
			0,
			{bs: newCol, d: s.d, e: s.e, b: newOffset, b6: newRow, P: s.P});
	};
};
...
var $elm$parser$Parser$Advanced$chompIf = F2(
	function (isGood, expecting) {
		return function (s) {
			var newOffset = A3($elm$parser$Parser$Advanced$isSubChar, isGood, s.b, s.P);
			return _Utils_eq(newOffset, -1) ? A2(
				$elm$parser$Parser$Advanced$Bad,
				false,
				...
				true,
				0,
				{bs: 1, d: s.d, e: s.e, b: s.b + 1, b6: s.b6 + 1, P: s.P}) : A3(
				$elm$parser$Parser$Advanced$Good,
				true,
				0,
				{bs: s.bs + 1, d: s.d, e: s.e, b: newOffset, b6: s.b6, P: s.P}));
		};
	});
...
var $elm$parser$Parser$Advanced$chompWhileHelp = F5(
	function (isGood, offset, row, col, s0) {
		chompWhileHelp:
		while (true) {
			var newOffset = A3($elm$parser$Parser$Advanced$isSubChar, isGood, offset, s0.P);
			if (_Utils_eq(newOffset, -1)) {
				return A3(
					$elm$parser$Parser$Advanced$Good,
					_Utils_cmp(s0.b, offset) < 0,
					0,
					{bs: col, d: s0.d, e: s0.e, b: offset, b6: row, P: s0.P});
			} else {
...
var $elm$parser$Parser$Advanced$chompWhile = function (isGood) {
	return function (s) {
		return A5($elm$parser$Parser$Advanced$chompWhileHelp, isGood, s.b, s.b6, s.bs, s);
	};
};
		...
		return function (s0) {
			...
			if (_v1.$ === 1) {
				...
				return A3(
					...
					A2(
						func,
						A3($elm$core$String$slice, s0.b, s1.b, s0.P),
						a),
					s1);
			}
		};
...
var $elm$parser$Parser$DeadEnd = F3(
	function (row, col, problem) {
		return {bs: col, bZ: problem, b6: row};
	});
var $elm$parser$Parser$problemToDeadEnd = function (p) {
	return A3($elm$parser$Parser$DeadEnd, p.b6, p.bs, p.bZ);
};
...
var $elm$parser$Parser$Advanced$run = F2(
	function (_v0, src) {
		var parse = _v0;
		var _v1 = parse(
			{bs: 1, d: _List_Nil, e: 1, b: 0, b6: 1, P: src});
		if (!_v1.$) {
			var value = _v1.b;
...
var $justinmimbs$date$Date$language_en = {
	...
	a9: $justinmimbs$date$Date$monthToName,
	aO: A2(
		$elm$core$Basics$composeR,
...
var $author$project$Resume$formatDateRange = F2(
	function (start, maybeEnd) {
		return A2($justinmimbs$date$Date$format, 'MMMM Y', start) + (' - ' + function () {
			if (!maybeEnd.$) {
				var end = maybeEnd.a;
				return A2($justinmimbs$date$Date$format, 'MMMM Y', end);
			} else {
				return 'Present';
			}
		}());
...
var $author$project$Resume$paddingLeft = function (pad) {
	return $mdgriffith$elm_ui$Element$paddingEach(
		{bp: 0, bM: pad, b5: 0, ce: 0});
};
...
var $author$project$Resume$skillBadge = _List_fromArray(
	[
		$mdgriffith$elm_ui$Element$padding(4),
		$mdgriffith$elm_ui$Element$Background$color(
		A3($mdgriffith$elm_ui$Element$rgb255, 235, 235, 235)),
		$mdgriffith$elm_ui$Element$Border$rounded(5)
	]);
var $author$project$Resume$textStyle = $elm$core$Basics$append(
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$Font$color($author$project$Resume$fgColor),
	...
	function (attrs, children) {
		...
		if (spaced.$ === 1) {
			return A4(
				...
				A2(
					$elm$core$List$cons,
					$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.at + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.L + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.bk)))),
					A2(
						$elm$core$List$cons,
						$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$shrink),
						A2(
...
var $author$project$Resume$historyEntry = F5(
	function (title, dates, location, skills, content) {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
				[
					$author$project$Resume$paddingLeft(10),
					$mdgriffith$elm_ui$Element$spacing(5),
					$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
					$mdgriffith$elm_ui$Element$htmlAttribute(
				...
				[
					...
					_List_fromArray(
						[
							...
							$author$project$ElementFix$text(dates)),
							A2(
							$mdgriffith$elm_ui$Element$el,
							$author$project$Resume$textStyle(
								_List_fromArray(
							...
							$author$project$ElementFix$text(location))
						])),
					$elm$core$List$isEmpty(skills) ? $mdgriffith$elm_ui$Element$none : A2(
					$mdgriffith$elm_ui$Element$wrappedRow,
					_List_fromArray(
						[
					...
					A2(
						...
						function (skill) {
							return A2(
								$mdgriffith$elm_ui$Element$el,
								$author$project$Resume$skillBadge,
								$author$project$ElementFix$text(skill));
						},
						skills)),
					content
				]));
...
var $author$project$Resume$educationToHistory = function (entry) {
	return A5(
		...
		entry.bI,
		A2($author$project$Resume$formatDateRange, entry.H, entry.D),
		entry.E,
		_List_Nil,
...
var $author$project$Experience$employmentList = _List_fromArray(
	[
		...
		D: $elm$core$Maybe$Just(
			A3($justinmimbs$date$Date$fromCalendarDate, 2019, 8, 1)),
		E: 'Nizhniy Novgorod, Russia',
		ab: 'Embedded developer (C/C++)',
		ac: _List_fromArray(
			['C/C++', 'Embedded', 'Networking', 'Buildroot', 'RTOS']),
		H: A3($justinmimbs$date$Date$fromCalendarDate, 2018, 2, 1)
	},
		{
		T: 'XDSoft',
		V: _List_fromArray(
		...
		D: $elm$core$Maybe$Just(
			A3($justinmimbs$date$Date$fromCalendarDate, 2021, 2, 1)),
		E: 'Novosibirsk, Russia',
		ab: 'Blockchain developer (Rust)',
		ac: _List_fromArray(
			['Rust', 'Tokio', 'GitLab CI', 'Exonum']),
		H: A3($justinmimbs$date$Date$fromCalendarDate, 2019, 8, 1)
	},
		{
		T: 'Gear',
		V: _List_fromArray(
		...
		D: $elm$core$Maybe$Just(
			A3($justinmimbs$date$Date$fromCalendarDate, 2021, 11, 1)),
		E: 'Moscow, Russia',
		ab: 'Blockchain developer (Rust)',
		ac: _List_fromArray(
			['Rust', 'WebAssembly', 'Substrate', 'API design']),
		H: A3($justinmimbs$date$Date$fromCalendarDate, 2021, 7, 1)
	},
		{
		T: 'Bictory',
		V: _List_fromArray(
		...
		D: $elm$core$Maybe$Just(
			A3($justinmimbs$date$Date$fromCalendarDate, 2022, 9, 1)),
		E: 'Remote',
		ab: 'Smart contract developer (Rust)',
		ac: _List_fromArray(
			['Rust', 'Smart Contracts', 'WebAssembly', 'Concordium', 'API design']),
		H: A3($justinmimbs$date$Date$fromCalendarDate, 2022, 0, 1)
	},
		{
		T: 'NEOM',
		V: _List_Nil,
		...
		ac: _List_fromArray(
			['Rust', 'Substrate']),
		H: A3($justinmimbs$date$Date$fromCalendarDate, 2023, 9, 1)
	},
		{
		T: 'TONOMUS-Milqia',
		V: _List_Nil,
		...
		ab: 'Co-Founder',
		ac: _List_Nil,
		H: A3($justinmimbs$date$Date$fromCalendarDate, 2023, 10, 1)
	}
	]);
...
var $author$project$Resume$employmentToHistory = function (entry) {
	return A5(
		$author$project$Resume$historyEntry,
		entry.ab + (' at ' + entry.T),
		A2($author$project$Resume$formatDateRange, entry.H, entry.D),
...
var $author$project$Resume$extraEntry = F2(
	function (title, content) {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
...
var $author$project$Resume$extrasTextStyle = $elm$core$Basics$append(
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$Font$color($author$project$Resume$fgColor),
			$mdgriffith$elm_ui$Element$Font$size(13),
...
var $author$project$Resume$skillColumns = F2(
	function (relevant, secondary) {
		var totalLength = $elm$core$List$length(relevant) + $elm$core$List$length(secondary);
		var secondaryFormatted = A2(
			$elm$core$List$map,
			A2(
				$elm$core$Basics$composeL,
				$mdgriffith$elm_ui$Element$el($author$project$Resume$skillBadge),
				$author$project$ElementFix$text),
			secondary);
		var relevantFormatted = A2(
			$elm$core$List$map,
			A2(
				$elm$core$Basics$composeL,
				$mdgriffith$elm_ui$Element$el(
					A2($elm$core$List$cons, $mdgriffith$elm_ui$Element$Font$bold, $author$project$Resume$skillBadge)),
				$author$project$ElementFix$text),
			relevant);
		var leftLength = ((totalLength / 2) | 0) + A2($elm$core$Basics$modBy, 2, totalLength);
		var joined = A2($elm$core$List$append, relevantFormatted, secondaryFormatted);
		return A2(
			$mdgriffith$elm_ui$Element$row,
			...
			_List_fromArray(
				[
					...
					A2($elm$core$List$take, leftLength, joined)),
					A2(
					$mdgriffith$elm_ui$Element$column,
					_List_fromArray(
						[
					...
					A2($elm$core$List$drop, leftLength, joined))
				]));
	});
		...
		[
			...
			_List_fromArray(
				[
					...
					A2(
						...
						$author$project$Experience$about)),
					A2(
					$author$project$Resume$bodyPart,
					'Employment history',
					A2(
						...
						A2(
							$elm$core$List$map,
							$author$project$Resume$employmentToHistory,
							$elm$core$List$reverse(
								A2(
									$elm$core$List$sortBy,
									function (e) {
										return $justinmimbs$date$Date$toRataDie(e.H);
									},
			...
			_List_fromArray(
				[
					...
					'Skills',
					A2(
						$author$project$Resume$skillColumns,
						_List_fromArray(
							['Rust', 'WebAssembly', 'Substrate', 'Smart Contracts', 'Blockchain', 'Linux', 'UNIX']),
						_List_fromArray(
							['API design', 'Git', 'GitLab CI', 'Tokio', 'x86 ASM', 'Elm', 'Golang', 'Lua', 'C']))),
					A2(
					$author$project$Resume$extraEntry,
					'Contacts',
					A2(
						...
						_List_fromArray(
							[
								...
								$author$project$Resume$paddingLeft(5)
							]),
						_List_fromArray(
							[
								A2(
								...
								_List_fromArray(
									[
										...
										$author$project$Resume$extrasTextStyle(
											_List_fromArray(
												[$mdgriffith$elm_ui$Element$Font$bold])),
										$author$project$ElementFix$text('E-mail')),
										A2(
										$mdgriffith$elm_ui$Element$link,
										_List_Nil,
										{
											u: A2(
												...
												$author$project$Resume$extrasTextStyle(
													_List_fromArray(
														[$mdgriffith$elm_ui$Element$alignRight])),
												$author$project$ElementFix$text('mikhail.pogretskiy@gmail.com')),
											A: 'mailto:mikhail.pogretskiy@gmail.com'
										})
									])),
								A2(
								$mdgriffith$elm_ui$Element$column,
								...
								_List_fromArray(
									[
										...
										$author$project$Resume$extrasTextStyle(
											_List_fromArray(
												[$mdgriffith$elm_ui$Element$Font$bold])),
										$author$project$ElementFix$text('Mobile')),
										A2(
										$mdgriffith$elm_ui$Element$el,
										$author$project$Resume$extrasTextStyle(_List_Nil),
										$author$project$ElementFix$text('+995 (595) 46 56 29'))
									]))
							]))),
					A2(
					$author$project$Resume$extraEntry,
					...
					A2(
						...
						$author$project$Resume$extrasTextStyle(_List_Nil),
						{
							u: A2(
								...
								$author$project$ElementFix$text('anfid.github.io')),
							A: 'https://anfid.github.io/'
						})),
					A2(
					$author$project$Resume$extraEntry,
					'Links',
							...
							u: A2(
								...
								_List_fromArray(
									[
										...
										{V: 'GitHub logo', P: '/assets/GitHub-Mark-32px.png'}),
										A2(
										$mdgriffith$elm_ui$Element$el,
										$author$project$Resume$extrasTextStyle(_List_Nil),
										$author$project$ElementFix$text('Anfid'))
									])),
							A: 'https://github.com/Anfid'
						})),
					A2(
					$author$project$Resume$extraEntry,
					'Languages',
					A2(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$author$project$ElementFix$text('English'),
										A2(
										$mdgriffith$elm_ui$Element$el,
										_List_fromArray(
											[
										...
										$author$project$ElementFix$text('Fluent'))
									])),
								A2(
								$mdgriffith$elm_ui$Element$row,
								_List_fromArray(
								...
								_List_fromArray(
									[
										$author$project$ElementFix$text('Russian'),
										A2(
										$mdgriffith$elm_ui$Element$el,
										_List_fromArray(
											[
										...
										$author$project$ElementFix$text('Native'))
									])),
								A2(
								$mdgriffith$elm_ui$Element$row,
								_List_fromArray(
								...
								_List_fromArray(
									[
										$author$project$ElementFix$text('Spanish'),
										A2(
										$mdgriffith$elm_ui$Element$el,
										_List_fromArray(
											[
										...
										$author$project$ElementFix$text('Basic'))
									]))
							])))
				]))
		]));
...
var $author$project$Resume$heading = A2(
	$mdgriffith$elm_ui$Element$row,
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$spacing(20),
			$mdgriffith$elm_ui$Element$width(
			A2($mdgriffith$elm_ui$Element$minimum, 470, $mdgriffith$elm_ui$Element$fill))
		]),
	_List_fromArray(
		[
			A2(
			...
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$Background$image('/assets/resume_photo_smol.jpeg')
				]),
			{V: '', P: '/assets/resume_photo.png'}),
			A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					...
					$author$project$ElementFix$text('Mikhail Pogretskiy')),
					A2(
					$mdgriffith$elm_ui$Element$el,
					$author$project$Resume$textStyle(_List_Nil),
					$author$project$ElementFix$text('Rust developer'))
				]))
		]));
var $author$project$Resume$contentView = A2(
	$mdgriffith$elm_ui$Element$column,
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$width(
	...
	_List_fromArray(
		[$author$project$Resume$heading, $author$project$Resume$body]));
					...
					A2(
						...
						A2(
							...
							A2(
								...
								A2(
									$elm$core$List$cons,
									$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.L),
									attrs)))))),
			$mdgriffith$elm_ui$Internal$Model$Unkeyed(
				_List_fromArray(
					[label])));
	});
var $author$project$Styles$icon = F2(
	function (style, attributes) {
		if (!style) {
			return A2(
				$elm$core$List$cons,
...
var $author$project$Styles$fgFadedColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 150, 150, 150);
	} else {
...
var $author$project$Styles$heading = F3(
	function (style, level, attributes) {
		var _v0 = function () {
			switch (level) {
				case 1:
					return _Utils_Tuple3(
				...
				case 3:
					return _Utils_Tuple3(
						$author$project$Styles$fgFadedColor(style),
						22,
						20);
				case 4:
					return _Utils_Tuple3(
		...
		return _Utils_ap(
			_List_fromArray(
				[
					...
					{bp: 0, bM: padding, b5: 0, ce: 0}),
					$mdgriffith$elm_ui$Element$Region$heading(
					A3($elm$core$Basics$clamp, 1, 5, level))
				]),
			attributes);
	});
var $author$project$Main$chapter = F4(
	function (heading, style, level, content) {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
				[
...
var $author$project$Styles$linkColor = function (style) {
	if (!style) {
		...
		return A3($mdgriffith$elm_ui$Element$rgb255, 51, 102, 153);
	}
};
var $author$project$Styles$linkFocusColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 109, 173, 176);
	} else {
		return A3($mdgriffith$elm_ui$Element$rgb255, 81, 132, 203);
	}
};
var $author$project$Styles$linkPressColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 131, 165, 152);
	} else {
		return A3($mdgriffith$elm_ui$Element$rgb255, 31, 72, 123);
	}
};
var $mdgriffith$elm_ui$Element$Font$underline = $mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.d0);
var $author$project$Styles$link = F2(
	function (style, attributes) {
		return _Utils_ap(
			_List_fromArray(
				[
					$mdgriffith$elm_ui$Element$Font$color(
					$author$project$Styles$linkColor(style)),
					$mdgriffith$elm_ui$Element$Font$underline,
					$mdgriffith$elm_ui$Element$mouseDown(
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Font$color(
							$author$project$Styles$linkPressColor(style))
						])),
					$mdgriffith$elm_ui$Element$mouseOver(
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Font$color(
							$author$project$Styles$linkFocusColor(style))
						])),
					$mdgriffith$elm_ui$Element$focused(
					_List_fromArray(
						[
...
var $author$project$Main$link = F3(
	function (style, label, url) {
		return A2(
			$mdgriffith$elm_ui$Element$link,
			A2($author$project$Styles$link, style, _List_Nil),
			{
				u: A2(
					$mdgriffith$elm_ui$Element$paragraph,
					_List_Nil,
					_List_fromArray(
						[
							$author$project$ElementFix$text(label)
						])),
				A: url
			});
	});
var $mdgriffith$elm_ui$Internal$Flag$fontAlignment = $mdgriffith$elm_ui$Internal$Flag$flag(12);
var $mdgriffith$elm_ui$Element$Font$alignLeft = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$fontAlignment, $mdgriffith$elm_ui$Internal$Style$classes.dQ);
...
var $author$project$Main$index = function (model) {
	return A2(
		$mdgriffith$elm_ui$Element$column,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$ElementFix$text('Hello, my name is Mikhail Pogretskiy. I am Rust software developer, and this page is created as a ' + 'general overview of my software development background.')
					])),
				A4(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$author$project$ElementFix$text('My primary commercial Rust experience is related to blockchain, smart contracts and WASM. ' + ('Other than that, I extend my knowledge in other areas by creating personal ' + ('pet-projects with various levels of polish. They are usually quite limited ' + 'due to lack of time and the amount of these projects.')))
							])),
						A2(
						$mdgriffith$elm_ui$Element$paragraph,
						A2($author$project$Styles$paragraph, model.a, _List_Nil),
						_List_fromArray(
							[
								$author$project$ElementFix$text('I have experience in creating and designing multithreaded systems, and I enjoy writing portable, ' + 'highly optimized code.')
							]))
					])),
				A4(
				$author$project$Main$chapter,
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$author$project$ElementFix$text('You can find '),
								A3($author$project$Main$link, model.a, 'my resume here', '/resume'),
								$author$project$ElementFix$text('. If you need to download it, '),
								A2(
								$mdgriffith$elm_ui$Element$downloadAs,
								A2($author$project$Styles$link, model.a, _List_Nil),
								{
									bw: 'Mikhail-Pogretskiy.pdf',
									u: $author$project$ElementFix$text('click here'),
									A: '/assets/resume.pdf'
								}),
								$author$project$ElementFix$text('.')
							])),
						...
						_List_fromArray(
							[
								$author$project$ElementFix$text('Also check out my '),
								A3($author$project$Main$link, model.a, 'personal projects', '/projects'),
								$author$project$ElementFix$text('.')
							]))
					]))
			]));
...
var $author$project$Main$notPrintable = $mdgriffith$elm_ui$Element$htmlAttribute(
...
var $author$project$Styles$smallButtonColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 50, 50, 50);
	} else {
		return A3($mdgriffith$elm_ui$Element$rgb255, 241, 231, 189);
	}
};
var $author$project$Styles$smallButtonHoverColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 60, 60, 60);
	} else {
		return A3($mdgriffith$elm_ui$Element$rgb255, 246, 236, 194);
	}
};
var $author$project$Styles$smallButtonPressColor = function (style) {
	if (!style) {
		return A3($mdgriffith$elm_ui$Element$rgb255, 45, 45, 45);
	} else {
		return A3($mdgriffith$elm_ui$Element$rgb255, 236, 226, 184);
	}
};
var $author$project$Styles$smallButton = F2(
	function (style, attributes) {
		return _Utils_ap(
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$Background$color(
					$author$project$Styles$smallButtonColor(style)),
					$mdgriffith$elm_ui$Element$Font$color(
					$author$project$Styles$fgColor(style)),
					$mdgriffith$elm_ui$Element$Font$family(
					_List_fromArray(
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Background$color(
							$author$project$Styles$smallButtonPressColor(style))
						])),
					$mdgriffith$elm_ui$Element$mouseOver(
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Background$color(
							$author$project$Styles$smallButtonHoverColor(style))
						])),
					$mdgriffith$elm_ui$Element$focused(
					_List_fromArray(
						[
...
var $author$project$Main$smallMonochromeIconButton = F5(
	function (style, logo, desc, buttonText, url) {
		return A2(
			$mdgriffith$elm_ui$Element$link,
			A2(
				$author$project$Styles$smallButton,
				style,
				_List_fromArray(
					[
				...
				u: A2(
					...
					_List_fromArray(
						[
							...
							$mdgriffith$elm_ui$Element$el,
							_List_Nil,
							$author$project$ElementFix$text(buttonText))
						])),
				A: url
			});
	});
var $author$project$Main$liveDemoButton = F2(
	function (style, url) {
		return A5($author$project$Main$smallMonochromeIconButton, style, '/assets/play-button-arrowhead.png', 'Live demo icon', 'Live demo', url);
	});
var $author$project$Main$sourceButton = F2(
	function (style, url) {
		return A5($author$project$Main$smallMonochromeIconButton, style, '/assets/GitHub-Mark-32px.png', 'GitHub logo', 'Source', url);
	});
var $author$project$Main$techStack = F2(
	function (style, stack) {
		return A2(
			...
			A2(
				...
				A2(
					$mdgriffith$elm_ui$Element$el,
					A3($author$project$Styles$heading, style, 3, _List_Nil),
					$author$project$ElementFix$text('Stack')),
				A2(
					$elm$core$List$map,
					function (el) {
						return A2(
							...
							_List_fromArray(
								[
									...
									$mdgriffith$elm_ui$Element$el,
									_List_Nil,
									$author$project$ElementFix$text(el))
								]));
					},
					stack)));
	});
							...
							[
								...
								_List_fromArray(
									[
										...
										{V: 'dnd-stuff screenshot', P: '/assets/projects/dnd-stuff_demo.jpg'}),
										A2(
										$mdgriffith$elm_ui$Element$row,
										_List_fromArray(
											[
										...
										_List_fromArray(
											[
												A2($author$project$Main$liveDemoButton, model.a, '/projects/dnd-stuff/index.html'),
												A2($author$project$Main$sourceButton, model.a, 'https://github.com/Anfid/dnd-stuff')
											])),
										A2(
										...
										_List_fromArray(
											[
												$author$project$ElementFix$text('An Elm + Rust web project inspired by '),
												A3($author$project$Main$link, model.a, 'rumkin.com/reference/dnd/diestats.php', 'http://rumkin.com/reference/dnd/diestats.php'),
												$author$project$ElementFix$text('. It shows probabilities of getting each value with specified dice combination. ' + ('I was disappointed that it was practically impossible to see probabilities for ' + ('more complex combinations due to poor performance and decided to try and create ' + 'probabitity calculator with best performance possible.')))
											])),
										A2(
										$mdgriffith$elm_ui$Element$paragraph,
										A2($author$project$Styles$paragraph, model.a, _List_Nil),
										_List_fromArray(
											[
												$author$project$ElementFix$text('This project uses Elm for page rendering and Rust library compiled into WASM ' + ('for performance-sensitive calculations. Minimal necessary JS glue-code is confined ' + 'to index.html file.'))
											]))
									]))
							])),
						A4(
							...
							[
								...
								_List_fromArray(
									[
										...
										{V: 'coppe-wm screenshot', P: '/assets/projects/coppe_demo.jpg'}),
										A2(
										$mdgriffith$elm_ui$Element$row,
										_List_fromArray(
											[
										...
										_List_fromArray(
											[
												A2($author$project$Main$sourceButton, model.a, 'https://github.com/Anfid/coppe-wm')
											])),
										A2(
										$author$project$Main$techStack,
										model.a,
										...
										_List_fromArray(
											[
												$author$project$ElementFix$text('Window manager for X11 fully configurable with standalone WASM plugins. It is ' + ('intended to be as feature-rich as possible, but only the most basic functionality ' + 'is implemented at the moment.'))
											]))
									]))
							])),
						A4(
							...
							[
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												A2($author$project$Main$sourceButton, model.a, 'https://github.com/Anfid/water')
											])),
										A2(
										$author$project$Main$techStack,
										model.a,
										...
										_List_fromArray(
											[
												$author$project$ElementFix$text('A simple Rust synthesyser for a MIDI keyboard. Currenly only CLI is available,' + ('UI is unimplemented. It is able to produce square sound wave with correct frequency ' + 'for each key pressed.'))
											]))
									]))
							]))
					])),
							...
							[
								...
								_List_fromArray(
									[
										...
										{V: 'GItask screenshot', P: '/assets/projects/GItask_demo.jpg'}),
										A2(
										$mdgriffith$elm_ui$Element$row,
										_List_fromArray(
											[
										...
										_List_fromArray(
											[
												A2($author$project$Main$sourceButton, model.a, 'https://github.com/Anfid/GItask')
											])),
										A2(
										$author$project$Main$techStack,
										model.a,
										...
										_List_fromArray(
											[
												$author$project$ElementFix$text('Small isometric C++ game written on SDL. Although project is very old, it is a fun demo still.')
											]))
									]))
							]))
					])),
							...
							[
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												A2($author$project$Main$sourceButton, model.a, 'https://github.com/Anfid/Anfid.github.io')
											])),
										A2(
										$author$project$Main$techStack,
										model.a,
										...
										_List_fromArray(
											[
												$author$project$ElementFix$text('Well, it\'s this page.')
											]))
									]))
							]))
					]))
...
var $mdgriffith$elm_ui$Element$Border$roundEach = function (_v0) {
	var topLeft = _v0.d_;
	var topRight = _v0.d$;
	var bottomLeft = _v0.cN;
	var bottomRight = _v0.cO;
	return A2(
		$mdgriffith$elm_ui$Internal$Model$StyleClass,
		$mdgriffith$elm_ui$Internal$Flag$borderRound,
		A3(
...
var $author$project$Styles$sansSerif = F3(
	function (style, size, attributes) {
		return _Utils_ap(
			_List_fromArray(
				[
					$mdgriffith$elm_ui$Element$Font$color(
...
var $mdgriffith$elm_ui$Element$scrollbars = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$overflow, $mdgriffith$elm_ui$Internal$Style$classes.dx);
var $author$project$Resume$view = A2(
	$mdgriffith$elm_ui$Element$el,
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$padding(50),
	...
	$author$project$Resume$contentView);
	...
	switch (_v0) {
		case 0:
			return A2(
				...
				_List_fromArray(
					[
						A2($author$project$Main$bar, model, _List_Nil),
						$author$project$Main$index(model)
					]));
		case 1:
			return A2(
				$mdgriffith$elm_ui$Element$column,
				...
				_List_fromArray(
					[
							...
							[
								...
								A2($author$project$Styles$button, model.a, _List_Nil),
								{
									...
									u: A2(
										...
										{V: 'Download', P: '/assets/download.png'}),
									A: '/assets/resume.pdf'
								}),
								A2(
								$mdgriffith$elm_ui$Element$link,
						...
						A2($mdgriffith$elm_ui$Element$map, $author$project$Main$LinkClicked, $author$project$Resume$view))
					]));
		case 2:
			return A2(
				$mdgriffith$elm_ui$Element$el,
					...
					[
						...
						A2(
							...
							A2(
								...
								_List_fromArray(
									[
										$author$project$Main$notPrintable,
										$mdgriffith$elm_ui$Element$alignRight,
									...
									])),
							{
								u: A2(
									...
									A2(
										...
										{V: 'Print', P: '/assets/print.png'})),
								bT: $elm$core$Maybe$Just($author$project$Main$Print)
							}))
					]),
				A2(
					$mdgriffith$elm_ui$Element$el,
					...
					A2($mdgriffith$elm_ui$Element$map, $author$project$Main$LinkClicked, $author$project$Resume$contentView)));
		case 3:
			return A2(
				$mdgriffith$elm_ui$Element$column,
				_List_fromArray(
				...
				_List_fromArray(
					[
						...
						A3(
							...
							_List_fromArray(
								[$mdgriffith$elm_ui$Element$centerX])),
						$author$project$ElementFix$text('404')),
						A2(
						$mdgriffith$elm_ui$Element$el,
						A3(
							$author$project$Styles$sansSerif,
							...
							_List_fromArray(
								[$mdgriffith$elm_ui$Element$centerX])),
						$author$project$ElementFix$text('Not Found')),
						A2(
						$mdgriffith$elm_ui$Element$link,
						A2(
							$author$project$Styles$button,
								...
								[$mdgriffith$elm_ui$Element$centerX])),
						{
							u: A2(
								...
								_List_fromArray(
									[
										$author$project$ElementFix$text('Let\'s go home?')
									])),
							A: '/'
						})
					]));
...
var $mdgriffith$elm_ui$Internal$Model$optionsToRecord = function (options) {
	...
	return andFinally(
		A3(
			$elm$core$List$foldr,
			combine,
			{c1: $elm$core$Maybe$Nothing, c5: $elm$core$Maybe$Nothing, dk: $elm$core$Maybe$Nothing},
			options));
};
			...
			A2(
				...
				$mdgriffith$elm_ui$Internal$Model$htmlClass(
					A2(
						...
						' ',
						_List_fromArray(
							[$mdgriffith$elm_ui$Internal$Style$classes.dv, $mdgriffith$elm_ui$Internal$Style$classes.cA, $mdgriffith$elm_ui$Internal$Style$classes.dC]))),
...
var $andrewMacmurray$elm_simple_animation$Internal$Ease$Cubic = F4(
	function (a, b, c, d) {
		return {$: 0, a: a, b: b, c: c, d: d};
	});
var $andrewMacmurray$elm_simple_animation$Internal$Ease$easeInQuad = A4($andrewMacmurray$elm_simple_animation$Internal$Ease$Cubic, 0.11, 0, 0.5, 0);
var $andrewMacmurray$elm_simple_animation$Simple$Animation$easeInQuad = $andrewMacmurray$elm_simple_animation$Internal$Animation$Ease($andrewMacmurray$elm_simple_animation$Internal$Ease$easeInQuad);
var $andrewMacmurray$elm_simple_animation$Simple$Animation$Stepped = $elm$core$Basics$identity;
...
var $andrewMacmurray$elm_simple_animation$Simple$Animation$toAnimation = function (_v0) {
	...
	return A3(
		...
		$andrewMacmurray$elm_simple_animation$Simple$Animation$totalDuration(s.aT),
		s.dr,
		A2($andrewMacmurray$elm_simple_animation$Simple$Animation$toFrames, s.aS, s.aT));
};
		...
		return $andrewMacmurray$elm_simple_animation$Simple$Animation$toAnimation(
			{
				...
				aT: _List_fromArray(
					[
						A2($andrewMacmurray$elm_simple_animation$Simple$Animation$step, o.c_, to_)
					])
			});
	});
...
var $andrewMacmurray$elm_simple_animation$Internal$Animation$defaults = {aK: $elm$core$Maybe$Nothing, ak: false, al: $elm$core$Maybe$Nothing, aw: false, aV: $elm$core$Maybe$Nothing};
...
var $andrewMacmurray$elm_simple_animation$Internal$Animation$AlternateReverse_ = 2;
...
var $andrewMacmurray$elm_simple_animation$Internal$Animation$toDirection = function (opts) {
	return (opts.ak && opts.aw) ? $elm$core$Maybe$Just(2) : (opts.aw ? $elm$core$Maybe$Just(1) : (opts.ak ? $elm$core$Maybe$Just(0) : $elm$core$Maybe$Nothing));
};
var $andrewMacmurray$elm_simple_animation$Internal$Animation$renderOptions_ = function (opts) {
	return _List_fromArray(
		[
			A3($andrewMacmurray$elm_simple_animation$Internal$Animation$renderOption, 'animation-delay', $andrewMacmurray$elm_simple_animation$Internal$Unit$ms, opts.aK),
			A3($andrewMacmurray$elm_simple_animation$Internal$Animation$renderOption, 'animation-timing-function', $andrewMacmurray$elm_simple_animation$Internal$Ease$toString, opts.aV),
			A3($andrewMacmurray$elm_simple_animation$Internal$Animation$renderOption, 'animation-iteration-count', $andrewMacmurray$elm_simple_animation$Internal$Animation$renderIteration, opts.al),
			A3(
				...
				switch (transform.a.$) {
					...
					default:
						...
						return _Utils_update(
							combined,
							{
								aY: $elm$core$Maybe$Just(n)
							});
				}
		}
	});
var $andrewMacmurray$elm_simple_animation$Internal$Transform$empty = {aP: $elm$core$Maybe$Nothing, aR: $elm$core$Maybe$Nothing, aW: $elm$core$Maybe$Nothing, aX: $elm$core$Maybe$Nothing, aY: $elm$core$Maybe$Nothing};
...
var $andrewMacmurray$elm_simple_animation$Internal$Transform$translate_ = function (combined) {
	var _v0 = _Utils_Tuple3(combined.aX, combined.aW, combined.aY);
	if (!_v0.a.$) {
		var _v1 = _v0.a.a;
		var x_ = _v1.a;
		var y_ = _v1.b;
		...
		A2(
			...
			_List_fromArray(
				[
					...
					A2($andrewMacmurray$elm_simple_animation$Internal$Transform$render_, $andrewMacmurray$elm_simple_animation$Internal$Transform$scale_, combined.aR),
					A2($andrewMacmurray$elm_simple_animation$Internal$Transform$render_, $andrewMacmurray$elm_simple_animation$Internal$Transform$rotate_, combined.aP)
				])));
};
...
var $author$project$Main$styleStack = F5(
	function (fView, x, y, count, model) {
		return (count > 0) ? A2(
			$elm$core$List$cons,
			$mdgriffith$elm_ui$Element$inFront(
				A5(
					$andrewMacmurray$elm_simple_animation$Simple$Animation$Animated$ui,
					{cF: $mdgriffith$elm_ui$Element$behindContent, c6: $mdgriffith$elm_ui$Element$html, c7: $mdgriffith$elm_ui$Element$htmlAttribute},
					$mdgriffith$elm_ui$Element$el,
					A3(
						$andrewMacmurray$elm_simple_animation$Simple$Animation$fromTo,
						{
							c_: $author$project$Main$styleSwitchDuration,
							dr: _List_fromArray(
								[$andrewMacmurray$elm_simple_animation$Simple$Animation$easeInQuad])
						},
						_List_fromArray(
							[
								A2(
								$andrewMacmurray$elm_simple_animation$Simple$Animation$Property$property,
								'clip-path',
								'circle(0px at ' + ($elm$core$String$fromInt(x) + ('px ' + ($elm$core$String$fromInt(y) + 'px)'))))
							]),
						_List_fromArray(
							[
								A2(
								$andrewMacmurray$elm_simple_animation$Simple$Animation$Property$property,
								'clip-path',
								'circle(' + ($elm$core$String$fromInt(
									2 * A2($elm$core$Basics$max, model.au.aM, model.au.aC)) + ('px at ' + ($elm$core$String$fromInt(x) + ('px ' + ($elm$core$String$fromInt(y) + 'px)'))))))
							])),
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
					...
					fView(model))),
			A5(
				$author$project$Main$styleStack,
				fView,
				x,
				y,
				count - 1,
				_Utils_update(
					model,
					{
						a: $author$project$Styles$flip(model.a)
					}))) : _List_Nil;
	});
var $author$project$Main$styleableView = F2(
	function (fView, model) {
		return A2(
			$mdgriffith$elm_ui$Element$el,
			_Utils_ap(
				_List_fromArray(
	...
	return {
		cH: _List_fromArray(
			[
				...
				A2($author$project$Main$styleableView, $author$project$Main$body, model))
			]),
		dZ: 'Mikhail Pogretskiy'
	};
};
var $author$project$Main$main = $elm$browser$Browser$application(
	{db: $author$project$Main$init, $7: $author$project$Main$UrlChanged, dp: $author$project$Main$LinkClicked, dK: $author$project$Main$subscriptions, d1: $author$project$Main$update, d2: $author$project$Main$view});
